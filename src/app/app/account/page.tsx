    ...
    <div className="flex flex-col lg:flex-row gap-8 h-[calc(100vh-8rem)] max-w-[1260px] mx-auto">
      ...
      <div className="lg:hidden">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-[#1E293B] border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-medium text-[#1A1A1A] dark:text-gray-100 w-full"
        >
          <span>{getSectionTitle()}</span>
          <svg className="h-4 w-4 ml-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={isMobileMenuOpen ? "M5 15l7-7 7 7" : "M19 9l-7 7-7-7"} />
          </svg>
        </button>
      </div>
      ...
      {isMobileMenuOpen && (
        <>
          ...
          />
          <aside className="fixed inset-y-0 left-0 w-64 bg-white dark:bg-[#1E293B] z-50 lg:hidden shadow-lg overflow-y-auto">
            <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between sticky top-0 bg-white dark:bg-[#1E293B]">
              <h2 className="text-sm font-semibold text-[#1A1A1A] dark:text-gray-100">설정</h2>
              <button
                onClick={() => setIsMobileMenuOpen(false)}
      ...
      <aside className="hidden lg:block w-64 shrink-0 border-r border-gray-200 dark:border-gray-700">
